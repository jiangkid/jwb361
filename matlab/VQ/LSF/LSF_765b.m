...
LSF_CB_6b = codeBookTrain(residStage1, stage2_b);
%Vector Quantization
for i = 1:length
    distance = pdist2(residStage1(i,:), LSF_CB_6b);
    [value, idx] = min(distance);
    VQ2(i) = idx;
    residStage2(i,:) = residStage1(i,:) - LSF_CB_6b(idx,:);  
end
disp('stage2 completed');

%stage3, train with residual of stage2
LSF_CB_5b = codeBookTrain(residStage2, stage3_b);
save('LSF_CB_765b.mat', 'codebook1', 'codebook2', 'codebook3');
%Vector Quantization
for i = 1:length
    distance = pdist2(residStage2(i,:), LSF_CB_5b);
    [value, idx] = min(distance);
    VQ3(i) = idx;
    residStage3(i,:) = residStage2(i,:) - LSF_CB_5b(idx,:);  
end
