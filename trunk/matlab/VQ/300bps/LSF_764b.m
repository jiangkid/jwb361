...
LSF_CB_764_7 = codeBookTrain(train_signal, stage1_b);
%Vector Quantization
for i = 1:length
    distance = pdist2(train_signal(i,:), LSF_CB_764_7);
    [value, idx] = min(distance);
    VQ1(i) = idx;
    residStage1(i,:) = train_signal(i,:) - LSF_CB_764_7(idx,:);    
end
disp('stage1 completed');

%stage2, train with residual of stage1
LSF_CB_764_6 = codeBookTrain(residStage1, stage2_b);
%Vector Quantization
for i = 1:length
    distance = pdist2(residStage1(i,:), LSF_CB_764_6);
    [value, idx] = min(distance);
    VQ2(i) = idx;
    residStage2(i,:) = residStage1(i,:) - LSF_CB_764_6(idx,:);  
end
disp('stage2 completed');

%stage3, train with residual of stage2
LSF_CB_764_4 = codeBookTrain(residStage2, stage3_b);
save('LSF_CB_764b.mat', 'LSF_CB_764_7', 'LSF_CB_764_6', 'LSF_CB_764_4');
%Vector Quantization
for i = 1:length
    distance = pdist2(residStage2(i,:), LSF_CB_764_4);
    [value, idx] = min(distance);
    VQ3(i) = idx;
    residStage3(i,:) = residStage2(i,:) - LSF_CB_764_4(idx,:);  
end
disp('LSF_764b completed');
save('LSF_764b.mat');
